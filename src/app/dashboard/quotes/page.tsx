                ...
                {isDbError && (
                    <div className="mb-6 text-sm text-text-muted p-md bg-black/20 rounded inline-block text-left">
                        ...
                        <ul className="list-disc list-inside mt-2">
                            <li>Ensure MongoDB is running (port 27017).</li>
                            <li>Ensure the FastAPI backend is running (port 8000).</li>
    ...
    return (
        <div className="flex flex-col gap-lg">
            <div className="flex justify-between items-center mb-md">
                <h1 className="text-[1.5rem] font-bold text-text-main">Quotes</h1>
            </div>

            {loading ? (
                <div className="text-center p-8 text-text-muted">Loading quotes...</div>
            ) : (
                <>
                        ...
                        <table className="w-full border-collapse min-w-[1000px]">
                            <thead>
                                <tr>
                                    ...
                                    <th className="text-left p-md text-text-muted font-medium border-b border-border whitespace-nowrap">Address</th>
                                    <th className="text-left p-md text-text-muted font-medium border-b border-border whitespace-nowrap">Info</th>
                            ...
                            <tbody>
                                {quotes.map((quote, index) => (
                                    <tr key={quote._id}>
                                        <td className="p-md border-b border-white/5 text-text-main align-middle whitespace-nowrap">{(page - 1) * pageSize + index + 1}</td>
                                        <td className="p-md border-b border-white/5 text-text-main align-middle whitespace-nowrap">{quote.name}</td>
                                        <td className="p-md border-b border-white/5 text-text-main align-middle whitespace-nowrap">{quote.company_name || '-'}</td>
                                        <td className="p-md border-b border-white/5 text-text-main align-middle whitespace-nowrap">{quote.phone_number}</td>
                                        <td className="p-md border-b border-white/5 text-text-main align-middle whitespace-nowrap">{quote.email_address}</td>
                                        <td className="p-md border-b border-white/5 text-text-main align-middle whitespace-nowrap">{quote.company_address}</td>
                                        <td className="p-md border-b border-white/5 text-text-main align-middle whitespace-nowrap">{quote.additional_information || '-'}</td>
                                        <td className="p-md border-b border-white/5 text-text-main align-middle whitespace-nowrap">
